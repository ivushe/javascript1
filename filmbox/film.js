...
detailFilmu.querySelector('.card-title').textContent = selectedFilm.nazev;
detailFilmu.querySelector('.card-text').textContent = selectedFilm.popis;
const posterImage = detailFilmu.querySelector('.img-fluid'); //css trida pro plakat
posterImage.src = selectedFilm.plakat.url;
posterImage.width = selectedFilm.plakat.sirka;
posterImage.height = selectedFilm.plakat.vyska;

const premiera = document.querySelector('#premiera');
const premieraDen = dayjs(selectedFilm.premiera);
const formattedPremiera = premieraDen.format('D. M. YYYY');
premiera.textContent = formattedPremiera; //update obsahu prvku premiera

const daysDiff = premieraDen.diff(dayjs(), 'days');
const premiereStatusElement = document.querySelector('#premiera');

const absoluteDaysDiff = Math.abs(daysDiff);

if (daysDiff < 0) {
	premiereStatusElement.innerHTML += ` Od premiéry uběhlo <strong>${absoluteDaysDiff}</strong> ${pluralize(absoluteDaysDiff, 'den', 'dny', 'dní')} `;
} else if (daysDiff === 0) {
	premiereStatusElement.innerHTML += ' Premiéra je dnes';
} else {
	premiereStatusElement.innerHTML += ` Premiéra je za <strong>${absoluteDaysDiff}</strong> ${pluralize(absoluteDaysDiff, 'den', 'dny', 'dní')}`;
}

function pluralize(count, singular, plural1, plural2) {
	if (count === 1) {
		return singular;
...
let selectedRating = 0; // vychozi hodnota

// zvyrazneni a odebrani zvyrazneni hvezdicek
...
// listener click pro vsechny hvezdicky
document.querySelectorAll('.fa-star').forEach((star, index) => {
...
//listener mouseenter pro vsechny hvezdicky
document.querySelectorAll('.fa-star').forEach((star, index) => {
	star.addEventListener('mouseenter', () => highlightStars(index + 1));
});

// listener mouseleave pro vsechny hvezdicky
...
// po reloadu stranky nacteni puvodni hodnoty
highlightStars(selectedRating);
